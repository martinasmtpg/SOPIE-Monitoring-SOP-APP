...
{
    public class ApvInputsController : Controller
    {
        ...
        public ActionResult Index()
        {
            ...
            var lasthistory = _context.HistoryTransaction.GroupBy(x => x.DetailTransaction.TransactionId).Select(x => x.OrderByDescending(z => new { z.DetailTransactionId, z.Id }).FirstOrDefault()).ToList().Where(y => y.ActivityTimelineId == 2 && y.ProgresStatusId == 6).ToList();
            var idhistory = lasthistory.Select(x => x.Id).ToList();
            var iddetail = lasthistory.Select(x => x.DetailTransactionId).ToList();
            var detail = _context.DetailTransaction.Include("Transaction.Bab.Book").Where(x => iddetail.Contains(x.Id) && x.Transaction.Bab.Kelompok.Singkatan == currentUser.Kelompok).ToList();
        ...
        public JsonResult Approve(int id, string keterangan)
        {
            var manager = new UserManager<ApplicationUser>(new UserStore<ApplicationUser>(new ApplicationDbContext()));
            var currentUser = manager.FindById(User.Identity.GetUserId());
            ...
            historyTransaction.ApproveKeterangan = keterangan;
            ...
            historyTransaction2.DetailTransactionId = id;
            historyTransaction2.ActivityTimelineId = 3;
            historyTransaction2.AktivitasTargetId = 3;
        ...
        public JsonResult Reject(int id, string keterangan)
        {
            var manager = new UserManager<ApplicationUser>(new UserStore<ApplicationUser>(new ApplicationDbContext()));
            var currentUser = manager.FindById(User.Identity.GetUserId());

            var historyTransaction = _context.HistoryTransaction.Where(x => x.DetailTransactionId == id).OrderByDescending(x => x.Id).FirstOrDefault();
            historyTransaction.ProgresStatusId = 5;
            historyTransaction.ApproveDate = DateTime.Now;
            historyTransaction.ApproverId = currentUser.Id;
            historyTransaction.ApproveKeterangan = keterangan;
            _context.Entry(historyTransaction).State = EntityState.Modified;
