...
{
    public class CheckVerifikasisController : Controller
    {
        ApplicationDbContext _context = new ApplicationDbContext();
        // GET: CheckVerifikasis
        public ActionResult Index()
        {
            var manager = new UserManager<ApplicationUser>(new UserStore<ApplicationUser>(new ApplicationDbContext()));
            var currentUser = manager.FindById(User.Identity.GetUserId());

            var lasthistory = _context.HistoryTransaction.GroupBy(x => x.DetailTransaction.TransactionId).Select(x => x.OrderByDescending(z => new { z.DetailTransactionId, z.Id }).FirstOrDefault()).ToList().Where(y => y.ActivityTimelineId == 3 && y.ProgresStatusId == 4).ToList();
        ...
        public JsonResult Approve(int id, string keterangan)
        {
            ...
            historyTransaction.CheckerDate = DateTime.Now;
            historyTransaction.CheckerId = currentUser.Id;
            historyTransaction.CheckKeterangan = keterangan;
